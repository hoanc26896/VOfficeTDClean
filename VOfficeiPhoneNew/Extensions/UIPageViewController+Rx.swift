...
extension Reactive where Base: UIPageViewController {
    ...
    var didFinishAnimating: Observable<(pageViewController: UIPageViewController, finished: Bool, previousViewControllers: [UIViewController], completed: Bool)> {
        delegate.methodInvoked(#selector(UIPageViewControllerDelegate.pageViewController(_:didFinishAnimating:previousViewControllers:transitionCompleted:)))
            .map { ($0[0] as! UIPageViewController, $0[1] as! Bool, $0[2] as! [UIViewController], $0[3] as! Bool) }
    }
